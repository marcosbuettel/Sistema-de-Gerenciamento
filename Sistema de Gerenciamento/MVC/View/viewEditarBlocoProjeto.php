<?php 
	//PÁGINA PARA EDIÇÃO DO BLOCO DO PROJETO
...
?>
	...
	<section class="linha-tempo container" >
		...
		<div class="janela-editar-bloco-calendario">
			<?php 
				$verificaBlocoProjeto = $pdo->prepare("SELECT * FROM bloco_projeto WHERE posicao_bloco_projeto = '$idBlocoProjeto' AND id_projeto = '$idCalendario'");
				$verificaBlocoProjeto->execute();
				$totalBlocoProjeto = $verificaBlocoProjeto->fetchAlL(); 

				$idBlocoProjetoSelecionado = $totalBlocoProjeto[0]['id_bloco_projeto'];	
			?>
			...
			<div class="info-cadastro-bloco-calendario">				
				...
				<form method="POST" action="../Model/modelEditarBlocoProjeto.php?id=<?php echo $idBlocoProjetoSelecionado. "&idP=" .$idCalendario?>">

					<label>Descrição: </label>
					<textarea name="descricao-bloco-projeto"><?php echo $totalBlocoProjeto[0]['desc_bloco_projeto']?></textarea>
					<br><br>
					<label>Status: </label>
